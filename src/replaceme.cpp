...
#include <replaceme/dynparamConfig.h>
...
class Replaceme : public nodelet::Nodelet {
...
private:
  ...
  typedef replaceme::dynparamConfig                              Config;
  typedef dynamic_reconfigure::Server<replaceme::dynparamConfig> ReconfigureServer;
  boost::recursive_mutex                                                      mutex_dynamic_reconfigure_;
  boost::shared_ptr<ReconfigureServer>                                        reconfigure_server_;
  void                                                                        callbackDynamicReconfigure(Config& config, uint32_t level);
  replaceme::dynparamConfig                                      last_drs_config_;
...
void Replaceme::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh, "Replaceme");
  ...
  c_loop_               = 0;
  ROS_INFO_STREAM_ONCE("[Replaceme]: " << n_waypoints_ << " waypoints loaded");
  ROS_INFO_STREAM_ONCE("[Replaceme]: " << _n_loops_ << " loops requested");
  ...
  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[Replaceme]: failed to load non-optional parameters!");
    ros::shutdown();
  }
  ...
  sh_control_manager_diag_ = mrs_lib::SubscribeHandler<mrs_msgs::ControlManagerDiagnostics>(shopts, "control_manager_diagnostics_in",
                                                                                            &Replaceme::callbackControlManagerDiag, this);
  ...
  timer_publish_dist_to_waypoint_ = nh.createTimer(ros::Rate(_rate_timer_publish_dist_to_waypoint_), &Replaceme::timerPublishDistToWaypoint, this);

  timer_check_subscribers_ = nh.createTimer(ros::Rate(_rate_timer_check_subscribers_), &Replaceme::timerCheckSubscribers, this);

  // you can disable autostarting of the timer by the last argument
  timer_publisher_reference_ = nh.createTimer(ros::Rate(_rate_timer_publisher_reference_), &Replaceme::timerPublishSetReference, this, false, false);

  // | --------------- initialize service servers --------------- |

  srv_server_start_waypoints_following_ = nh.advertiseService("start_waypoints_following_in", &Replaceme::callbackStartWaypointFollowing, this);
  srv_server_stop_waypoints_following_  = nh.advertiseService("stop_waypoints_following_in", &Replaceme::callbackStopWaypointFollowing, this);
  srv_server_fly_to_first_waypoint_     = nh.advertiseService("fly_to_first_waypoint_in", &Replaceme::callbackFlyToFirstWaypoint, this);
  ...
  ReconfigureServer::CallbackType f = boost::bind(&Replaceme::callbackDynamicReconfigure, this, _1, _2);
  ...
  ROS_INFO_ONCE("[Replaceme]: initialized");
...
void Replaceme::callbackControlManagerDiag(const mrs_msgs::ControlManagerDiagnostics::ConstPtr diagnostics) {
  ...
  ROS_INFO_ONCE("[Replaceme]: Received first control manager diagnostics msg");
  ...
  ROS_INFO("[Replaceme]: Distance to waypoint: %.2f", dist);
  ...
  if (have_goal_ && !diagnostics->tracker_status.have_goal) {
    ...
    if (dist < _waypoint_desired_dist_) {
      ...
      ROS_INFO("[Replaceme]: Waypoint reached.");

      /* start idling at the reached waypoint */
      is_idling_ = true;

      ros::NodeHandle nh("~");
      timer_idling_ = nh.createTimer(ros::Duration(_waypoint_idle_time_), &Replaceme::timerIdling, this,
                                     true);  // the last boolean argument makes the timer run only once

      ROS_INFO("[Replaceme]: Idling for %.2f seconds.", _waypoint_idle_time_);
    }
  }
}
...
void Replaceme::timerPublishSetReference([[maybe_unused]] const ros::TimerEvent& te) {
  ...
  if (idx_current_waypoint_ >= n_waypoints_) {
    ...
    ROS_INFO("[Replaceme]: Finished loop %d/%d", c_loop_, _n_loops_);

    if (c_loop_ >= _n_loops_) {

      ROS_INFO("[Replaceme]: Finished %d loops of %d waypoints.", _n_loops_, n_waypoints_);

      if (_land_end_) {
        ROS_INFO("[Replaceme]: Calling land service.");
        std_srvs::Trigger srv_land_call;
        srv_client_land_.call(srv_land_call);
      }

      ROS_INFO("[Replaceme]: Shutting down.");
      ros::shutdown();
      return;

    } else {
      ROS_INFO("[Replaceme]: Starting loop %d/%d", c_loop_ + 1, _n_loops_);
      idx_current_waypoint_ = 0;
    }
  }
  ...
  ROS_INFO("[Replaceme]: Flying to waypoint %d: x: %.2f y: %.2f z: %.2f heading: %.2f", idx_current_waypoint_ + 1, new_waypoint.reference.position.x,
...
void Replaceme::timerPublishDistToWaypoint([[maybe_unused]] const ros::TimerEvent& te) {
...
void Replaceme::timerCheckSubscribers([[maybe_unused]] const ros::TimerEvent& te) {
  ...
  if (!sh_odometry_.hasMsg()) {
    ROS_WARN_THROTTLE(1.0, "[Replaceme]: Not received uav odom msg since node launch.");
  }

  if (!sh_control_manager_diag_.hasMsg()) {
    ROS_WARN_THROTTLE(1.0, "[Replaceme]: Not received tracker diagnostics msg since node launch.");
  }
}
...
void Replaceme::timerIdling([[maybe_unused]] const ros::TimerEvent& te) {

  ROS_INFO("[Replaceme]: Idling finished");
  is_idling_ = false;
}
...
bool Replaceme::callbackStartWaypointFollowing([[maybe_unused]] std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {

  if (!is_initialized_) {

    res.success = false;
    res.message = "Waypoint flier not initialized!";
    ROS_WARN("[Replaceme]: Cannot start waypoint following, nodelet is not initialized.");
    return true;
  }
  ...
  if (waypoints_loaded_) {
    ...
    ROS_INFO("[Replaceme]: Starting waypoint following.");
    ...
    ROS_WARN("[Replaceme]: Cannot start waypoint following, waypoints are not set.");
    res.success = false;
...
bool Replaceme::callbackStopWaypointFollowing([[maybe_unused]] std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {

  if (!is_initialized_) {

    res.success = false;
    res.message = "Waypoint flier not initialized!";
    ROS_WARN("[Replaceme]: Cannot stop waypoint following, nodelet is not initialized.");
    return true;
  }

  timer_publisher_reference_.stop();

  ROS_INFO("[Replaceme]: Waypoint following stopped.");
...
bool Replaceme::callbackFlyToFirstWaypoint([[maybe_unused]] std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {
  ...
  if (waypoints_loaded_) {
    ...
    ROS_INFO_STREAM_THROTTLE(1.0, "[Replaceme]: " << ss.str());
    ...
    ROS_WARN("[Replaceme]: Cannot fly to first waypoint, waypoints not loaded!");
...
void Replaceme::callbackDynamicReconfigure([[maybe_unused]] Config& config, [[maybe_unused]] uint32_t level) {
...
std::vector<mrs_msgs::Reference> Replaceme::matrixToPoints(const Eigen::MatrixXd& matrix) {
...
void Replaceme::offsetPoints(std::vector<mrs_msgs::Reference>& points, const Eigen::MatrixXd& offset) {
...
double Replaceme::distance(const mrs_msgs::Reference& waypoint, const geometry_msgs::Pose& pose) {
...
PLUGINLIB_EXPORT_CLASS(replaceme::Replaceme, nodelet::Nodelet);
